...
void write_to_file(void *buf, int size, long filepos)
{
    ...
    MudFile file(mud->mailFile);

    if (!(mail_file = fopen(file.getCPath(), "r+b"))) {
        log("SYSERR: Unable to open mail file '%s'.", file.getCPath());
        no_mail = TRUE;
        return;
    }
...
int scan_file(void)
{
    ...
    if (!(mail_file = fopen(file.getCPath(), "r"))) {
...
SPECIAL(postmaster)
{
    ...
    if (no_mail) {
        send_to_char("К сожалению почтовые пересылки отключены.\r\n", ch);
        return (0);
    }
...
void postmaster_send_mail(struct char_data *ch, struct char_data *mailman, int cmd, char *arg)
{
    ...
    if (GET_LEVEL(ch) < MIN_MAIL_LEVEL) {
        send_to_charf(ch, "Вы должны достичь %d уровня, чтобы отправлять письма.\r\n",
                      MIN_MAIL_LEVEL);
    ...
    if (!*buf) {                /* you'll get no argument from me! */
        send_to_charf(ch, "Укажите кому отправить письмо.\r\n");

        return;
    }
    if (GET_GOLD(ch) < STAMP_PRICE) {
        send_to_charf(ch, "Отправка письма стоит %d %s.'\r\n"
                      "Но у Вас нет столько денег.\r\n",
                      STAMP_PRICE, desc_count(STAMP_PRICE, WHAT_MONEYu));

        return;
    }
    if ((recipient = get_id_by_name(buf)) < 0) {
        send_to_charf(ch, "Такого адреса в мире не существует.\r\n");
        return;
    }
    act("$n начал$g писать письмо.", TRUE, ch, 0, 0, TO_ROOM);
    send_to_charf(ch, "Отправка письма будет стоит %d %s.\r\n"
                  "Вы начали писать письмо (/s saves /h for help).\r\n",
...
void postmaster_receive_mail(struct char_data *ch, struct char_data *mailman, int cmd, char *arg)
{
    ...
    if (!has_mail(GET_IDNUM(ch))) {
        send_to_charf(ch, "Для Вас нет почтовых сообщений.\r\n");
        return;
    }
    while (has_mail(GET_IDNUM(ch))) {
        ...
        GET_OBJ_MAX(obj) = GET_OBJ_CUR(obj) = 100;
        GET_OBJ_COST(obj) = 30;
        GET_OBJ_TIMER(obj) = 1440;
        GET_OBJ_TEMP(obj) = 0;
